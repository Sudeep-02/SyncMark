...
from app.services.tag_service import add_tags_to_bookmark, remove_tags_from_bookmark, clear_all_tags
...
def add_tags(bookmark_id: UUID, body: TagModifyRequest, db: Session = Depends(get_session),
    ...
    return add_tags_to_bookmark(db, user_id, bookmark_id, body.tag_ids)
...
def remove_tags(bookmark_id: UUID, body: TagModifyRequest, db: Session = Depends(get_session),
                user_id: UUID = Depends(get_current_user)):
    return remove_tags_from_bookmark(db, user_id, bookmark_id, body.tag_ids)
...
def clear_tags(bookmark_id: UUID, db: Session = Depends(get_session),
               user_id: UUID = Depends(get_current_user)):
    return clear_all_tags(db, user_id, bookmark_id)
